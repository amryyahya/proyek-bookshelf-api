...
const getAllBooksHandler = (request, h) => {
  ...
  const isFinised = !!Number(finished);
  if (name !== undefined) {
    certainBooks = certainBooks.filter((b) => b
      .name.toUpperCase().includes(name.toUpperCase()));
  }
  if (reading !== undefined) {
    certainBooks = certainBooks.filter((b) => b.reading === isReading);
  }

  if (finished !== undefined) {
    certainBooks = certainBooks.filter((b) => b.finished === isFinised);
  }
  const response = h.response({
    status: 'success',
    data: {
